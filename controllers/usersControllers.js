...
exports.createNewUser = (req, res) => {
  ...
  if (!firstName || !lastName) return res.status(400).json({ error: "Le prénom et le nom de famille sont requis !" })
  if (typeof firstName !== "string") return res.status(400). json({ error: "That's a weird name !" })
  if (!isAlphanumeric(firstName)) return res.status(400). json({ error: "That name is not allowed !" })
